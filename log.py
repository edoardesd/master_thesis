...
def signal_handler():
	...
	subprocess.check_output(["mysqlimport --ignore-lines=1 --fields-terminated-by=, \
								...
								"+pwd+starting_day+"/"+starting_time+"/"+db_table+"_bluetooth.csv"], shell=True)

	subprocess.check_output(["mysqlimport --ignore-lines=1 --fields-terminated-by=, \
								--columns='mac_address,rasp,rx,timestamp' \
								--local -u "+db_user+" -h "+db_host+ " -p"+db_pass+" "+db_database+" \
								"+pwd+starting_day+"/"+starting_time+"/"+db_table+"_hcidump.csv"], shell=True)

	subprocess.check_output(["mysqlimport --ignore-lines=1 --fields-terminated-by=, \
								--columns='mac_address,rasp,rx,timestamp,sn' \
								--local -u "+db_user+" -h "+db_host+ " -p"+db_pass+" "+db_database+" \
								"+pwd+starting_day+"/"+starting_time+"/"+db_table+"_wifi.csv"], shell=True)
