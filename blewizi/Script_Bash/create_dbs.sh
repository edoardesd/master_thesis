...
for i in `seq 1 10`;
        do
        	for j in `seq 1 5`;
				...
				#mysql -u root -pblewizipass -D fingerprint_v3 -e  "CREATE TABLE h${i}_${j}_wifi (mac_address VARCHAR(30), rasp INT, rx INT, timestamp VARCHAR(30), sn INT);"
				#mysql -u root -pblewizipass -D fingerprint_v3 -e  "CREATE TABLE h${i}_${j}_bluetooth (mac_address VARCHAR(30), timestamp VARCHAR(30), rasp INT, echo_time FLOAT, rssi INT);"

                                mysql -u root -pblewizipass -D fingerprint_v3 -e "DELETE FROM h${i}_${j}_wifi WHERE (mac_address!='C4:43:8F:B3:0A:F7' AND mac_address!='C8:14:79:31:3C:2A' AND mac_address!='D8:90:E8:29:AD:3F');"
